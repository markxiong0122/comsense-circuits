...
To test whether commonsense signal concentrates in attention or MLP sublayers---as has been reported for physical-commonsense tasks on PIQA~\citep{bisk2020piqa} using GPT-2 Small~\citep{wang2022interpretability}---we decompose layer patching into separate \texttt{hook\_attn\_out} and \texttt{hook\_mlp\_out} interventions.
...
This differs from the MLP-dominance pattern reported for GPT-2 Small on PIQA~\citep{bisk2020piqa, geva2021transformer}, likely reflecting both architectural differences (Qwen3-8B uses grouped-query attention) and task differences (Com2Sense requires multi-hop causal/temporal reasoning vs.\ PIQA's single-step physical intuition).
